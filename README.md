...
| Nav link (logged out) | `Login` and `Register` |
| Nav link (logged in) | `Logout` |
...
- **Refresh required** (-0.5): After creating or deleting a post, the change must appear immediately without the user manually reloading the page. Re-fetch or update state client-side after each operation.
